...
**Entscheidung:**
- Flutters Standard-Ansatz wird verwendet: ARB-Dateien (`mobile_app/lib/l10n/app_de.arb` als Template/Default-Locale, `app_en.arb` als zweite Sprache) plus `flutter_localizations`/`intl` und codegenerierte `AppLocalizations` (`flutter gen-l10n`, gesteuert über `mobile_app/l10n.yaml`, `generate: true` in `pubspec.yaml`). Deutsch bleibt die Standard-/Primärsprache der App, Englisch ist als zweite ARB-Sprache angelegt.
- **Update (iter3, `feature/iter3-config-push-wiring`): Sprachumschalter ergänzt.** `AppSettings.languageCode` (`'de'`/`'en'`/`null` = Systemsprache) ist jetzt persistiert, einstellbar über den neuen `language_settings_screen.dart` (verlinkt aus `settings_screen.dart`). `app.dart`s `locale:` liest daraus über `resolveAppLocale()`: ein expliziter Wert gewinnt, `null` folgt der Systemsprache (`PlatformDispatcher.instance.locale`) sofern deren Sprache in `AppLocalizations.supportedLocales` enthalten ist, sonst Fallback auf Deutsch. Vorher war `locale:` hart auf `Locale('de')` gepinnt.
...
- **Alle übrigen `lib/features/**/presentation/*.dart`-Screens enthalten weiterhin hartkodierte deutsche Strings und sind NICHT vollständig migriert.** Explizite Liste (Stand dieser Runde), damit das nicht als "erledigt" missverstanden wird:
  ...
  - `lib/features/settings/presentation/sharing_settings_screen.dart` — **teilmigriert (iter3):** nur der neu verdrahtete Pairing-`ListTile` (Hinweistext bei fehlendem Relay-Server) nutzt jetzt `AppLocalizations`; der Rest des Screens (URL-Feld, Erklärtext, Speichern-Button) ist weiterhin hartkodiert deutsch.
  - `lib/features/settings/presentation/weather_settings_screen.dart` (falls/sobald vorhanden)
  - `lib/features/pairing/presentation/*.dart` — **teilmigriert (iter3):** `pairing_screen.dart` (nur der neue "Konfiguration an Gerät senden"-Button) und `config_push_confirmation_screen.dart` (nur der neu ergänzte Bestätigungstext) nutzen jetzt `AppLocalizations`; alle übrigen Strings in diesen zwei Dateien sowie `send_config_push_screen.dart`, `pairing_qr_display_screen.dart`, `pairing_scan_screen.dart`, `recovery_code_screen.dart`, `relay_server_setup_screen.dart`, `shared_album_view_screen.dart`, `upload_screen.dart` bleiben hartkodiert deutsch (siehe Task-Vorgabe: nur neu geschriebene UI wird migriert, nicht ohnehin unangetastete Bestandstexte).
  ...
  - `lib/features/settings/presentation/autostart_help_screen.dart` — **Ausnahme:** dieser Screen (Task "OEM-Autostart-Hinweis") wurde direkt mit `AppLocalizations` statt mit hartkodierten Strings angelegt, ist also bereits migriert.
  - `lib/features/settings/presentation/language_settings_screen.dart` — **neu (iter3), vollständig migriert:** direkt mit `AppLocalizations` angelegt (Radio-Optionen "Deutsch"/"English" bleiben bewusst als Eigennamen unübersetzt).

**Konsequenzen:**
- Neue zentrale Strings (Settings-Hub, Onboarding, Cache-Verwaltung, OEM-Autostart-Hinweis, Sprachauswahl, neue Pairing/Config-Push-UI) sind jetzt in `app_de.arb`/`app_en.arb` gepflegt statt im Code verstreut; jede neue ARB-Message trägt eine `@key`-Beschreibung für Übersetzer.
- Der Großteil der App bleibt bis auf Weiteres hartkodiert deutsch – funktional unverändert (keine Regressionen), aber die oben gelistete Restmenge muss in einer Folgerunde schrittweise nachgezogen werden, bevor die App wirklich mehrsprachig ist. Der neue Sprachumschalter funktioniert bereits vollständig für die bislang migrierten Strings; nicht migrierte Screens zeigen bei Sprachumschaltung auf Englisch weiterhin deutsche Texte.
