...
4. **Realtime via Socket.IO Rooms**: Nach erfolgreichem Pairing treten beide Frames dem Raum `pairing:<id>` bei – der Beitritt wird erst nach serverseitiger Mitgliedschaftsprüfung im Socket-Handshake gewährt (kein Raum-Beitritt allein durch Kenntnis der `pairing_id`). `routes/images.ts` broadcastet nach erfolgreichem Upload `album:updated` in diesen Raum (mit `.except()` auf den Uploader-Frame-Raum, sodass tatsächlich nur die übrigen Mitglieder benachrichtigt werden) und nach erfolgreichem `DELETE /images/:id` `album:imageDeleted` an alle Mitglieder. Zugriff auf die `io`-Instanz läuft über den bereits vorhandenen `getIo()`-Singleton-Getter aus `realtime/socket.ts` (siehe `docs/DECISIONS.md` ADR-008). Bekannte, bewusst offene Grenze: Mitgliedschaft wird nur beim `join_pairing`-Handshake geprüft, nicht erneut bei jedem Event – ein währenddessen entferntes Mitglied bleibt bis zum Trennen/Neuverbinden im Raum (dokumentiert im Code-Kommentar von `realtime/socket.ts`). Jeder verbundene Socket aktualisiert zusätzlich `frames.last_seen_at`, wodurch pro Mitglied ein Online/Offline-Status ableitbar ist.
...
- **Quotas/Missbrauchsschutz**: `MAX_UPLOAD_BYTES` (multer `limits.fileSize` ist der eigentliche, verlässliche Schutz – die Content-Length-Vorprüfung in `enforceMaxUploadSize` ist nur eine Optimierung gegen unnötiges Puffern und kann durch einen fehlenden/falschen Header umgangen werden), MIME-Whitelist, `storage_used_bytes` pro User (transaktional geprüft und bei jedem Lösch-Pfad wieder freigegeben – siehe `docs/DECISIONS.md` ADR-007 –, HTTP 413 bei Überschreitung), Bildlimit pro Pairing, Upload-Rate-Limit pro Frame, `config_pushes.ciphertext` ist auf 64 KiB begrenzt (Schutz gegen unbegrenztes Storage-Wachstum über diese Tabelle) und kann vom Ziel-Frame explizit per `POST /config-push/:id/reject` abgelehnt werden (analog zu `/ack`).
- **Blob-Refcount/Storage-Accounting bei Löschung**: `DELETE /pairings/:id`, das Admin-Löschen eines Users/Bildes und `DELETE /images/:id` lesen vor dem eigentlichen Löschen/der `ON DELETE CASCADE` alle betroffenen `images.content_hash` aus und dekrementieren `blobs.refcount` sowie `storage_used_bytes` über eine gemeinsame Hilfsfunktion (`storage/imageCleanup.ts`), innerhalb derselben Transaktion wie das Löschen selbst. `commitBlob()` (Upload-Pfad) läuft ebenfalls innerhalb derselben Transaktion wie der `images`-Insert, statt – wie ursprünglich – davor. Details/Begründung: `docs/DECISIONS.md` ADR-007.
